...
open class RNChartViewBase: UIView, ChartViewDelegate {
  ...
  func setLegend(_ config: NSDictionary) {
    ...
    if json["horizontalAlignment"].string != nil {
      legend.horizontalAlignment = BridgeUtils.parseLegendHorizontalAlignment(
        json["horizontalAlignment"].stringValue)
    }

    if json["verticalAlignment"].string != nil {
      legend.verticalAlignment = BridgeUtils.parseLegendVerticalAlignment(
        json["verticalAlignment"].stringValue)
    }
  ...
  func setChartDescription(_ config: NSDictionary) {
    ...
    if json["positionX"].number != nil && json["positionY"].number != nil {
      chartDescription.position = CGPoint(
        x: CGFloat(truncating: json["positionX"].numberValue),
        y: CGFloat(truncating: json["positionY"].numberValue))
    }
  ...
  func setAnimation(_ config: NSDictionary) {
    ...
    let durationX = json["durationX"].double != nil ? json["durationX"].doubleValue / 1000.0 : 0
    let durationY = json["durationY"].double != nil ? json["durationY"].doubleValue / 1000.0 : 0
    ...
    if durationX != 0 && durationY != 0 {
      chart.animate(
        xAxisDuration: durationX, yAxisDuration: durationY, easingOptionX: easingX,
        easingOptionY: easingY)
    } else if (durationX != 0) {
  ...
  func setCommonAxisConfig(_ axis: AxisBase, config: JSON) {
      ...
      for limitLineConfig in limitLinesConfig {
        ...
        if limitLineConfig["limit"].double != nil {
          ...
          if (limitLineConfig["valueTextColor"].int != nil) {
            limitLine.valueTextColor = RCTConvert.uiColor(
              limitLineConfig["valueTextColor"].intValue)
          }

          if (limitLineConfig["valueFont"].int != nil) {
            limitLine.valueFont = NSUIFont.systemFont(
              ofSize: CGFloat(limitLineConfig["valueFont"].intValue))
          }
          ...
          if limitLineConfig["labelPosition"].string != nil {
            limitLine.labelPosition = BridgeUtils.parseLimitlineLabelPosition(
              limitLineConfig["labelPosition"].stringValue)
          }
    ...
    if valueFormatter.array != nil {
      axis.valueFormatter = IndexAxisValueFormatter(
        values: valueFormatter.arrayValue.map({ $0.stringValue }))
      ...
      if "largeValue" == valueFormatter.stringValue {
        ...
        let timeUnit =
          config["timeUnit"].string != nil ? config["timeUnit"].stringValue : "MILLISECONDS"
        let locale = config["locale"].string
        axis.valueFormatter = CustomChartDateFormatter(
          pattern: valueFormatterPattern, since: since, timeUnit: timeUnit, locale: locale)
      } else {
  ...
  func setMarker(_ config: NSDictionary) {
    ...
    var markerTypeParam = "balloon"
    if json["type"].exists() {
      markerTypeParam = json["type"].stringValue
    }
    let markerType = MarkerType(rawValue: markerTypeParam)
  ...
  @objc public func chartValueSelected(
    _ chartView: ChartViewBase, entry: ChartDataEntry, highlight: Highlight
  ) {
  ...
  @objc public func chartScaled(
    _ chartView: ChartViewBase, scaleX: CoreGraphics.CGFloat, scaleY: CoreGraphics.CGFloat
  ) {
    sendEvent("chartScaled")
  }

  @objc public func chartTranslated(
    _ chartView: ChartViewBase, dX: CoreGraphics.CGFloat, dY: CoreGraphics.CGFloat
  ) {
  ...
  func sendEvent(_ action: String) {
    ...
    if chart is BarLineChartViewBase {
      ...
      if let handler = viewPortHandler {
        let center = barLineChart.valueForTouchPoint(
          point: handler.contentCenter, axis: YAxis.AxisDependency.left)
        dict["centerX"] = center.x
        dict["centerY"] = center.y

        let leftBottom = barLineChart.valueForTouchPoint(
          point: CGPoint(x: handler.contentLeft, y: handler.contentBottom),
          axis: YAxis.AxisDependency.left)

        let rightTop = barLineChart.valueForTouchPoint(
        ...
        if self.group != nil && self.identifier != nil {
          ChartGroupHolder.sync(
            group: self.group!, identifier: self.identifier!, scaleX: barLineChart.scaleX,
            scaleY: barLineChart.scaleY, centerX: center.x, centerY: center.y,
  ...
  override open func didSetProps(_ changedProps: [String]!) {
    ...
    if self.group != nil && self.identifier != nil && chart is BarLineChartViewBase {
      ChartGroupHolder.addChart(
        group: self.group!, identifier: self.identifier!, chart: chart as! BarLineChartViewBase,
